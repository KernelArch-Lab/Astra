...
hardware}. HongMeng~\cite{hongmeng} comes nearest from the kernel
side, and its authors report walking \emph{back} from chain-revocable
capabilities to plain address tokens because revocation cost hurt
their common case. We read that as a result about their point in the
design space rather than about revocation in general, and it is the
comparison this paper is most interested in
...
it by measurement: four reference transports---pipe(2), socketpair,
io\_uring, Aeron---run head-to-head in the same TSC domain on the same
machine. An eRPC harness ships with the artefact too, but it needs a
local eRPC build we do not assume, so we do not report it.
...
socketpair, io\_uring, and Aeron, with throughput,
revocation-latency, and perf-counter measurements, all driven by one
shell command (\Cref{sec:eval}).
...
fastpath. And validate is flat \textit{regardless of pool occupancy}
(\Cref{fig:figure-2}), which is the $O(1)$ claim made concrete---the
shape rather than the floor, since a call this short is measured close
to the resolution of the timing instrumentation
(\Cref{sec:eval:hw}).
